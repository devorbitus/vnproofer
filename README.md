...
Create initial cspell.json file from template (disabled if already exists). VNProofer will automatically add any defined character names to the wokspace dictionary list.

###### Update character exclusions

Add any new characters to workspace dictionary list that were not previously added.
...
Perform a full spell check of all rpy files using cSpell
...
- [ ] Ability to add file-level excluded words to the local dictionary if the exclusion is used in more than 1 file and remove the local exclusion from all files
- [ ] Ability to iterate over each misspelled word and choose how to handle it (add to file level exclusion, add to local dictionary)
- [ ] Ability to interate over all new dialogue (like a new pull request to review)
